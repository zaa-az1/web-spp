...
<body>
    ...
    <main class="container">
        ...
        <div class="toolbar-container" style="justify-content: flex-end;">
            <form method="GET" action="kelola-petugas.php" class="filter-form">
                <input type="text" name="search" placeholder="Cari nama petugas..." value="<?= htmlspecialchars($search) ?>" class="filter-input">
                <button type="submit" class="btn-cari">Cari</button>
                <?php if ($search !== ''): ?>
                    <a href="kelola-petugas.php" class="btn-reset">Reset</a>
                <?php endif; ?>
            </form>
        </div>
                ...
                <tbody>
                    <?php if (mysqli_num_rows($hasil_list) > 0): ?>
                        ...
                        <tr>
                            ...
                            <td><span class="user-role"><?= htmlspecialchars($row['level']) ?></span></td>
                            <td style="text-align: center;">
                                <a href="kelola-petugas.php?aksi=edit&id=<?= $row['id_petugas'] ?>" class="btn-action-edit">Edit</a>
                                <a href="kelola-petugas.php?aksi=hapus&id=<?= $row['id_petugas'] ?>" onclick="return confirm('Yakin hapus petugas ini?')" class="btn-action-delete">Hapus</a>
                            </td>
                        </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr><td colspan="4" style="text-align:center; padding: 25px; color: #888;">Data petugas tidak ditemukan.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
